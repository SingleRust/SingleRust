...
impl Pca {
    pub fn fit(&mut self, data: &Array2<f64>, thread_pool: &ThreadPool) -> anyhow::Result<()> {
        ...
        if self.center || self.scale {
            log!(Level::Debug, "Calculating the mean if the axis in a thread pool");
        ...
        let svd = thread_pool.install(|| centered_data.svd())?;
        let s = Array1::from_vec(svd.s);
        ...
        self.components = Some(svd.V.slice(s![.., ..n_components]).to_owned());
